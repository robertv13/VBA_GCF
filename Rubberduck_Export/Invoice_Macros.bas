...
Sub Invoice_New()
    ...
    If shInvoice.Range("B27").Value = False Then
        With shInvoice
            ...
            .Range("N3:N6").ClearContents 'Clear Dates & Rate
            .Range("I10:M46,O10:O46,N48,N49,N52").ClearContents
            'Assign Invoice number & increment Next Invoice number
            .Range("N6").Value = .Range("B21").Value 'Paste Invoice Number
            .Range("B21").Value = .Range("B21").Value + 1 'Increment Next Invoice number by 1
            .Range("B20").Value = ""
            .Range("B26").Value = False
            .Range("B24").Value = False
            .Range("B27").Value = True  'Brand new invoice
        End With
    Else
        MsgBox "La facture actuelle n'a pas �t� sauvegard�e. Il est inutile d'en cr�er une nouvelle !"
    End If
    If shInvoice.Range("B28").Value Then Debug.Print "Now exiting  - [Invoice_Macros] - Sub Invoice_New() - Facture # " & shInvoice.Range("N6").Value & " - " & shInvoice.Range("J3").Value & vbNewLine
End Sub
...
Sub Invoice_SaveUpdate()
    With shInvoice
        ...
        If .Range("B18").Value = Empty Then
            ...
            Exit Sub
        End If
        If .Range("N3").Value = Empty Then
            MsgBox "Veuillez vous assurer d'avoir une date de facture avant de sauvegarder la facture"
            Exit Sub
        End If
        If .Range("N4").Value = Empty Then
            MsgBox "Veuillez vous assurer d'avoir une date due avant de sauvegarder la facture"
            Exit Sub
        End If
        If .Range("N5").Value = Empty Then
            MsgBox "Veuillez vous assurer d'avoir un taux horaire avant de sauvegarder la facture"
            Exit Sub
        End If
        ...
        If .Range("B20").Value = Empty Then 'New Invoice
            ...
            If shInvoice.Range("B28").Value Then Debug.Print Tab(10); "Cas A (B20 = '""' ) alors InvRow est �tabli avec les lignes existantes: InvRow = " & InvRow
        Else 'Existing Invoice
            InvRow = .Range("B20").Value 'Set Existing Invoice Row
             If shInvoice.Range("B28").Value Then Debug.Print Tab(10); "Cas B (B20 <> '""') alors B20 est utilis�: InvRow = " & InvRow
        End If
        If shInvoice.Range("B28").Value Then Debug.Print Tab(5); "B20 = " & .Range("B20").Value & "   B21 (Next Invoice #) = " & .Range("B21").Value
        'Load data into InvList (Invoice Header)
        If shInvoice.Range("B28").Value Then Debug.Print Tab(5); "Facture # = " & Format(shInvoice.Range("N6").Value, "00000") & " et InvRow = " & InvRow & " - Posting; dans; InvoiceListing; """
        For InvCol = 2 To 12
            InvList.Cells(InvRow, InvCol).Value = .Range(InvList.Cells(1, InvCol).Value).Value 'Save data into Invoice List
            If shInvoice.Range("B28").Value Then Debug.Print Tab(10); "InvRow = " & InvRow & "   InvCol = " & InvCol & "   From Cell  = " & InvList.Cells(1, InvCol).Value & "   et la valeur = " & .Range(InvList.Cells(1, InvCol).Value).Value
        Next InvCol
        ...
        For InvItemRow = 10 To LastItemRow
            ...
            If shInvoice.Range("B28").Value Then Debug.Print Tab(15); "D�tail (InvItems) - B" & ItemDBRow & " = " & InvItems.Range("B" & ItemDBRow).Value
        ...
        Next InvItemRow
NoItems:
        .Range("B27").Value = False
        MsgBox "La facture '" & Format(.Range("N6").Value, "00000") & "' est enregistr�e." & vbNewLine & vbNewLine & "Le total de la facture est " & Trim(Format(.Range("N50").Value, "### ##0.00 $")) & " (avant les taxes)", vbOKOnly, "Confirmation d'enregistrement"
    End With
...
Sub Invoice_Load()
    ...
    With shInvoice
        ...
        With InvItems
            ...
            If shInvoice.Range("B28").Value Then Debug.Print "LastRow = " & LastRow & "   Copie de '" & "A3:G" & LastRow & "   Crit�re: " & .Range("L3").Value
...
Sub Retour_Gauche()
    If shInvoice.Range("B28").Value Then Debug.Print "Now entering - [Invoice_Macros] - Sub Retour_Gauche() @ " & Time
    shInvoice.Range("C1").Select
    shInvoice.Range("P15").Select
    If shInvoice.Range("B28").Value Then Debug.Print "Now exiting  - [Invoice_Macros] - Sub Retour_Gauche()" & vbNewLine
End Sub
