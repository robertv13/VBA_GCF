...
Sub BillingEntry_Load()
    ...
    With shInvoice
        If .Range("B3").Value = "" Then
            MsgBox "Veuillez s�lectionner une charge valide"
            Exit Sub
        End If
...
Sub BillingEntry_AddToInvoice()
    ...
    With shInvoice
        If .Range("B17").Value = Empty Then
            MsgBox "Vous devez s�lectionner une charge pour l'ajouter � la facture"
        End If
        ...
        If .Range("G" & SelRow).Value = "Yes" Then
            If MsgBox("Cette charge a d�j� �t� factur�. �tes-vous certain de vouloir l'ajouter � nouveau ?", vbYesNo, "Charge d�j� factur�e") = vbNo Then Exit Sub
        End If
...
NewRow:
            ...
            If InvRow = 36 Then 'Max Items
                MsgBox "Vous avez atteint le maximum d'entr�e sur cette facture"
                Exit Sub
            End If
        'End If
