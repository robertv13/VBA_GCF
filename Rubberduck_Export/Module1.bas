...
Sub Main() 'Création de la feuille suivante (Mois Suivant)
    ...
    ActiveSheet.Sheets(NbSheets).Copy After:=ActiveSheet.Sheets(NbSheets)
    ActiveSheet.Sheets(NbSheets + 1).Name = Year(NouveauMois) & "-" & Format$(Month(NouveauMois), "00")
    ...
    'TODO Is this comment still valid? => Détermine la date du dernier jour du nouveau mois
    ...
    MsgBox "Le nouvel onglet (" & ActiveSheet.Sheets(NbSheets + 1).Name & ") a été créé " & _
                                                            "avec succès"
    
End Sub

Sub InitialiseProchainMois(base As String, nouveau As String, m As Long)
...
Sub ConstruireLigneNormale(r As Long, d As Date)
...
Sub ConstruireLigneVide(r As Long)

    ActiveSheet.Range("A" & r & ":AA" & r).ClearContents
'    With ActiveSheet.Range("A" & r & ":AA" & r).Interior
'        .Pattern = xlNone
'        .TintAndShade = 0
...
Sub Set_Formats_Totals_Columns(r As Range)
