...
Sub FilterProfDate()
    ...
    'shFiltered.Activate
    'shFiltered.UsedRange.Offset(1, 0).ClearContents
    
    sh.UsedRange.AutoFilter 2, frmSaisieHeures.cmbProfessionnel.value
    sh.UsedRange.AutoFilter 3, frmSaisieHeures.txtDate.value
    sh.UsedRange.Select
    
    'La feuille Heures ne devrait contenir que les enregistrements filtrés"
    sh.UsedRange.Copy shFiltered.Range("A1")
    shFiltered.Activate
    
    'La feuille HeuresFiltered ne devrait contenir que les enregistrements filtrés dans Heures"
    'sh.Activate
    sh.AutoFilterMode = False
    sh.ShowAllData
    'La feuille Heures ne devrait plus avoir de filtres"
