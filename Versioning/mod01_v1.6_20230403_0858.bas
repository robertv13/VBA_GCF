...
Sub FilterHoursToExport()
    ...
    'Once filtered, 'HeuresAExporter' worksheet should only show the filtered records
    Dim shH2E As Worksheet
    Set shH2E = ThisWorkbook.Sheets("HeuresAExporter")
    shH2E.UsedRange.Clear
...
'        .AutoFilter Field:=9, Criteria1:=xlFilterThisWeek, Operator:=xlFilterDynamic

'        .AutoFilter Field:=2, Criteria1:="MFP"
'        .AutoFilter Field:=12, Criteria1:="FAUX"

'        .AutoFilter Field:=9, _
'            Operator:=xlFilterValues, _
'            Criteria1:=Array(3, CDate("03/24/2023 00:00:00"))
'        .AutoFilter Field:=12, Criteria1:="FAUX"
    End With
    
    sh.UsedRange.Select
    sh.UsedRange.Copy shH2E.Range("A1")
...
Sub ImportClientsList() '---------------------------- 'v1.0 - 2023-03-23 @ 07:40
    ...
    'ActiveSheet.ListObjects("tCharges").TotalsRowRange.Select
...
Sub AjouteLigneDetail()
    ...
    Dim sh As Worksheet
    Set sh = ThisWorkbook.Sheets("Heures")
    'Always delete the TotalRow, before adding a new row to the table
